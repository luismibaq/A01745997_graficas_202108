...
function main() 
{
    ...
    //Pacman
    let pacman = createPacman(gl,10);

    mat4.identity(modelViewMatrix);
    
    mat4.translate(modelViewMatrix, modelViewMatrix, [-1.5, 0.82, -4]);
...
function createRombo(gl) 
{
    ...
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(verts), gl.STATIC_DRAW);
    //utiliza 4 vertices con tamaño 3
    let rombo = {buffer:vertexBuffer, vertSize:3, nVerts:4, primtype:gl.TRIANGLE_STRIP};

    return rombo;
}
    //Crear del pacman
function createPacman(gl,space) 
{
    let vertexBuffer;
    vertexBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffer);
    ...
    for (let count = .45; count >= -.5; count -= .1/space)
    {
        verts.push(count, Math.sqrt(Math.abs(.25 - (count*count))), 0.0);
    }

    for (let count = -.5; count <= .45; count += .1/space)
    {
        verts.push(count, -Math.sqrt(Math.abs(.25 - (count*count))), 0.0);
    }

    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(verts), gl.STATIC_DRAW);
    // depende de la resolucion del pacman el número de vertices, y utiliza Triangle fan ... por que se me hizo mas bonito, pero puedes utilizar LINE LOOP
    let rombo = {buffer:vertexBuffer, vertSize:3, nVerts:space*20+1, primtype:gl.TRIANGLE_FAN};
